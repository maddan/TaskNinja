...
app.factory('Auth', function(FURL, $firebaseAuth, $firebase) {
	...
	auth.$onAuth(function(authData) {
		if(authData) {
			...
			//Get profile uid from firebase 
			Auth.user.profile = $firebase(ref.child('profile')
				.child(authData.uid)).$asObject();
		} else {
			if(Auth.user && Auth.user.profile) {
				Auth.uer.profile.$destroy();
			}
			angular.copy({}, Auth.user);
		}
	});

	//Create a get_gravatar function with MD5 email
